...
func (s *Scheduler) LoopHandleRegistCache(m *manager.Manager, facesetName string) {
	for {
		...
		if length == 0 {
			time.Sleep(time.Millisecond * time.Duration(m.CustConfig.RegistPeriodSec))
...
func (s *Scheduler) LoopHandleDetectCache(m *manager.Manager, facesetName string) {
	for {
		...
		if length == 0 {
			time.Sleep(time.Millisecond * time.Duration(m.CustConfig.DetectPeriodSec))
			//glog.Infof("the length is 0, continue")
